...
def show():
    ...
    if st.session_state.tab_kho == "tuoi":
        ...
        with c1:
            ...
            ten_kh = st.selectbox("Khách hàng", [x["ten"] for x in options])
        ...
        with c2:
            ...
            ten_go = st.selectbox("Loại gỗ", [x["ten_go"] for x in options_go])
            loai_go_id = next(x["id"] for x in options_go if x["ten_go"] == ten_go)

        ds = lay_kho_tuoi(khach_hang_id, loai_go_id)
        if len(ds) == 0:
            st.info("Kho tươi đang trống.")
            return

        df = pd.DataFrame(ds)
        df.columns = ["ID", "Ngày nhập", "Số phiếu", "Khách hàng", "Tên gỗ", "Ký hiệu", "Dày", "Rộng", "Dài", "Kg", "Thanh", "M³"]
        ...
        with c2:
            ...
            options_go = [{"id": None, "ten_go": "Tất cả"}]
            options_go.extend(ds_go)
            ten_go = st.selectbox("Loại gỗ", [x["ten_go"] for x in options_go], key="go_kho")
            loai_go_id = next(x["id"] for x in options_go if x["ten_go"] == ten_go)
        ...
        df = pd.DataFrame(ds)
        df.columns = ["ID", "Ngày nhập", "Số phiếu", "Khách hàng", "Tên gỗ", "Ký hiệu", "Dày", "Rộng", "Dài", "Kg", "Thanh", "M³", "Ngày vào hầm", "Ngày ra hầm"]
        df.drop(columns=["ID"], inplace=True)
        df.insert(0, "STT", range(1, len(df) + 1))
        df["Ngày nhập"] = pd.to_datetime(df["Ngày nhập"]).dt.strftime("%d/%m/%Y")
